...
filelist <- list.files(path = "E:/Surface data", pattern = ".*.csv")

get_df <- function(filename){
  x <- (paste0("E:/Surface data/", filename))
...
#windowpath <- unlist(lapply(filelist[keep], function(x){paste("E:/windows/",x)}))
#mapply(write.csv, coords, windowpath)

##IMPORT WINDOWS

window_files <- list.files(path = "E:/windows", pattern = ".*.csv")

get_df <- function(filename){
  x <- (paste0("E:/windows/", filename))
...
coords <- lapply(window_files, get_df)
...
compare_congas <- function(pp, theta){
  ...
  library(spatstat)
  t <- kppm(pp ~ 1, clusters = "Thomas", method = "mincon")
...
plot_probability <- function(df){
  ...
  cat(paste("Mewer congas than expected (p < 0.05):", range(sig_l$rs)[1], "mm -", range(sig_l$rs)[2], "mm"), sep = "\n")
